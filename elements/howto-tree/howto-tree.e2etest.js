...
describe('howto-tree', function() {
  ...
  it('should expand parent tree-items on [arrow right]', async function() {
    ...
    await helper.pressKeyUntil(this.driver, Key.ARROW_DOWN, `return document.activeElement.getAttribute('aria-activedescendant') === '${parentTreeItemID}';`);
    await this.driver.actions().sendKeys(Key.ARROW_RIGHT).perform();
    expect(await parentTreeItem.getAttribute('aria-expanded')).to.equal('true');
  });
  ...
  it('should collapse parent tree-items on [arrow left]', async function() {
    ...
    expect(await parentTreeItem.getAttribute('aria-expanded')).to.equal('false');
  });
  ...
  it('should make the first visible tree-item the activedescendant on [home]', async function() {
    ...
    const tree = await this.driver.findElement(By.css('howto-tree'));
    const activeDescendant = await tree.getAttribute('aria-activedescendant');
    const firstTreeItem = await this.driver.findElement(By.css('howto-tree-item')).getAttribute('id');
    expect(activeDescendant).to.equal(firstTreeItem);
  });
  ...
  it('should make the last visible tree-item the activedescendant on [end]', async function() {
    ...
    const tree = await this.driver.findElement(By.css('howto-tree'));
    const activeDescendant = await tree.getAttribute('aria-activedescendant');
    expect(activeDescendant).to.equal(await lastVisibleTreeItem.getAttribute('id'));
  });
  ...
  it('should select an item on [space]', async function() {
    ...
    expect(await firstTreeItem.getAttribute('aria-selected')).to.equal('true');
  });
  ...
  it('should select and expand parent tree-items on click', async function() {
    ...
    expect(await parentTreeItem.getAttribute('aria-expanded')).to.equal('true');
    expect(await parentTreeItem.getAttribute('aria-selected')).to.equal('true');
  });
  ...
  it('should collapse expanded parent tree-items on click', async function() {
    ...
    let parentTreeItemLabel = await this.driver.executeScript(_ =>
      document.querySelector('howto-tree-item-group > label')
    );
